...
class Tax extends REST_Controller {
    ...
    public function save_post(){
        ...
        if ($this->form_validation->run()) {
            ...
            $data['vat']        = ($this->input->post("vat") ?? 0);
            $data['sgst']       = ($this->input->post("sgst") ?? 0);
            $data['cgst']       = ($this->input->post("cgst") ?? 0);
            $data['is_default'] = ($this->input->post("is_default") ?? 0);  
                         
            $result             = $this->Taxmodel->save($data, $main_id);
            $this->response([
                'validate' => TRUE,
